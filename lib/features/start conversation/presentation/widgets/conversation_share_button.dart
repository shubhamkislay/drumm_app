  ...
  void generateShareableLink() async {
    try {
      final Map<String, String> metadataMap =
          conversation.toJson().map((key, value) {
      ...
      metadata = BranchContentMetaData()
        ..addCustomMetadata('conversation', metadataMap);

      buo = BranchUniversalObject(
          canonicalIdentifier: 'flutter/branch',
          title: "Join the conversation on Drumm!",
          imageUrl: imageUrl,
          contentDescription: unescape.convert(conversation.question ?? ""),
      ...
      if (response.success) {
        ...
        String articleLink =
            "${(conversation.title != null) ? unescape.convert(conversation.title ?? "") : unescape.convert(conversation.meta ?? "")}\n\nTap to join the conversation on Drumm.\n${response.result}";
